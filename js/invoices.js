...
function setupInvoicesTab() {
    ...
    const tab = document.getElementById('invoices-tab');
    tab.addEventListener('shown.bs.tab', function() {
        loadInvoicesForTable();
    });

    // Check if tab is already active on load (fixes refresh issue)
    ...
    // Fix for View Invoice Modal scrollbar issue
...
async function loadInvoicesForTable() {
    try {
        ...
        try {
            ...
            snapshot = await db.collection('invoices').orderBy('createdAt', 'desc').get();
        } catch (e) {
            console.warn('OrderBy query failed, falling back to standard fetch', e);
            // Fallback to getting all documents if index is missing or field is missing
        ...
        // Ensure sorting is correct (especially for fallback)
        ...
        <tr>
            ...
            <td>
                <div class="btn-group btn-group-sm">
                    ...
                    <button class="btn btn-outline-success btn-mark-paid" data-id="${invoice.id}" title="Mark as Paid">
