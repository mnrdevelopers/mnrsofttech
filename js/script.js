...
function syncSidebarTabs() {
    const sidebarTabs = document.querySelectorAll('#sidebarTabs .nav-link');
    ...
    sidebarTabs.forEach(tab => {
        tab.addEventListener('click', function(e) {
            ...
            if (targetId === '#products' && typeof loadProducts === 'function') {
                loadProducts();
            } else if (targetId === '#invoices' && typeof loadInvoicesForTable === 'function') {
                loadInvoicesForTable();
            } else if (targetId === '#customers' && typeof loadCustomers === 'function') {
                loadCustomers();
            } else if (targetId === '#dashboard' && typeof updateDashboard === 'function') {
                updateDashboard();
            } else if (targetId === '#expenses' && typeof loadExpenses === 'function') {
                loadExpenses();
            } else if (targetId === '#consolidate' && typeof loadCustomersForConsolidation === 'function') {
            ...
            // Update Breadcrumb
            document.getElementById('pageTitle').textContent = this.innerText.trim();
    ...
    window.addEventListener('hashchange', function() {
        ...
        if (hash) {
            const targetTab = document.querySelector(`#sidebarTabs [data-bs-target="#${hash}"]`);
            if (targetTab) {
                targetTab.click();
            }
        }
    ...
    if (initialHash) {
        const initialTab = document.querySelector(`#sidebarTabs [data-bs-target="#${initialHash}"]`);
        if (initialTab) {
            initialTab.click();
        }
    } else {
        ...
        if (activeTab) {
            document.getElementById('pageTitle').textContent = activeTab.innerText.trim();
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', function(e) {
            ...
            if (authSystem) {
                authSystem.updateUserProfile(displayName, photoURL);
            }
        });
    }
    ...
    if (headerThemeToggle) {
        headerThemeToggle.addEventListener('click', toggleHeaderTheme);
    }
    ...
    // Invoice list event delegation for actions
    ...
    // Payment Confirmation Modal Action
    const confirmPaymentBtn = document.getElementById('confirmPaymentBtn');
    if (confirmPaymentBtn) {
        confirmPaymentBtn.addEventListener('click', function() {
            if (invoiceIdToPay) {
                processInvoicePayment(invoiceIdToPay);
            }
        });
    }

    // Save Item button event delegation
    document.getElementById('itemsContainer').addEventListener('click', function(e) {
        const saveBtn = e.target.closest('.save-item');
        if (saveBtn) {
            saveItemAsProduct(saveBtn);
        }
    });
});
...
function loadSettings() {
    ...
    // Ensure symbol is correct based on currency code
    appSettings.currencySymbol = currencySymbols[appSettings.currency] || '₹';
    ...
    const emailNotif = document.getElementById('emailNotifToggle');
    if (emailNotif) emailNotif.checked = appSettings.emailNotifications;
    
    const browserNotif = document.getElementById('browserNotifToggle');
    if (browserNotif) browserNotif.checked = appSettings.browserNotifications;
...
function saveSettings() {
    ...
    appSettings.emailNotifications = document.getElementById('emailNotifToggle').checked;
    appSettings.browserNotifications = document.getElementById('browserNotifToggle').checked;
    appSettings.currency = document.getElementById('currencySelect').value;
    appSettings.currencySymbol = currencySymbols[appSettings.currency] || '₹';
    ...
    if (document.getElementById('invoicePreview').innerHTML.includes('invoice-template')) {
        generateInvoicePreview();
    }
    
    // Refresh dashboard charts if active
...
async function loadSavedProducts() {
    try {
        ...
        savedProducts = [];
        const datalist = document.getElementById('savedProductsList');
        datalist.innerHTML = '';
        ...
        snapshot.forEach(doc => {
            ...
            // Store price in label if you want it visible in dropdown, though value is what fills input
            // option.label = `₹${product.price}`; 
            datalist.appendChild(option);
        });
        console.log('Loaded saved products:', savedProducts.length);
    } catch (error) {
...
function addNewItemRow() {
    ...
    newItemRow.innerHTML = `
        ...
        <div class="item-col col-desc">
            <label>Description</label>
            <input type="text" class="form-control item-desc" placeholder="Description" required list="savedProductsList">
        </div>
        <div class="item-col col-hsn">
            <label>HSN/SAC</label>
            <input type="text" class="form-control item-hsn" placeholder="SAC">
        </div>
        ...
        <div class="item-col col-disc">
            <label>Disc %</label>
            <input type="number" class="form-control item-discount" placeholder="Disc %" min="0" max="100" step="0.1">
        </div>
        <div class="item-col col-warranty">
            ...
            <div class="warranty-wrapper">
                ...
                <input type="text" class="form-control custom-warranty-input mt-1" placeholder="Enter warranty details" style="display: none;">
            </div>
        </div>
        <div class="item-col col-action">
            <label class="d-none d-md-block">&nbsp;</label>
            <div class="item-actions">
                <button type="button" class="btn btn-outline-success save-item" title="Save Item"><i class="fas fa-save"></i></button>
    ...
    newItemRow.querySelector('.item-type').addEventListener('change', function() {
        const placeholder = this.value === 'product' ? 'HSN' : 'SAC';
        newItemRow.querySelector('.item-hsn').placeholder = placeholder;
    });

    // Add listener for description input (Autocomplete)
    newItemRow.querySelector('.item-desc').addEventListener('input', function() {
        const val = this.value;
        const product = savedProducts.find(p => p.name === val);
        if (product) {
            const row = this.closest('.item-row');
            row.querySelector('.item-type').value = product.type || 'service';
            ...
            row.querySelector('.item-type').dispatchEvent(new Event('change'));
            ...
            if (product.warranty) {
                if (['no-warranty', '7-days', '15-days', '1-month', '3-months', '6-months', '1-year'].includes(product.warranty)) {
                    ...
                    const customInput = row.querySelector('.custom-warranty-input');
...
async function saveItemAsProduct(btn) {
    const row = btn.closest('.item-row');
    const name = row.querySelector('.item-desc').value.trim();
    const type = row.querySelector('.item-type').value;
    const hsn = row.querySelector('.item-hsn').value.trim();
    const price = parseFloat(row.querySelector('.item-price').value) || 0;
    const warrantySelect = row.querySelector('.item-warranty').value;
    const customWarranty = row.querySelector('.custom-warranty-input').value.trim();
    
    if (!name) {
        showToast('Please enter a description to save', 'warning');
        return;
    }
    ...
    try {
        ...
        // Use name as ID to prevent duplicates easily, or auto-ID
        await db.collection('products').doc(name.replace(/\//g, '_')).set(productData);
        await loadSavedProducts(); // Reload list
        showToast('Item saved to products successfully', 'success');
    } catch (error) {
...
function collectInvoiceData() {
    ...
    itemRows.forEach(row => {
        ...
        if (description && price > 0) {
            const subLineTotal = quantity * price;
            const discountAmount = subLineTotal * (discount / 100);
            const total = subLineTotal - discountAmount;
            items.push({
                type,
                description,
                hsn,
...
async function saveInvoiceToFirebase() {
    ...
    try {
        ...
        // Handle Stock Updates (Deduct from inventory)
        await handleStockUpdates(invoiceToSave, isEditing ? currentEditingInvoiceId : null);
...
function populateItemRow(row, item) {
    ...
    if (typeSelect) {
        typeSelect.value = item.type || 'service';
        // Trigger placeholder update
        const placeholder = typeSelect.value === 'product' ? 'HSN' : 'SAC';
        if (hsnInput) hsnInput.placeholder = placeholder;
    }
            ...
            ${items.length > 0 ? `
                <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 14px;">
                    <thead>
                        <tr>
                            ...
                            <th style="background-color: #2c3e50; color: white; padding: 12px; text-align: center; border: 1px solid #ddd; width: 80px;">${hsnLabel}</th>
                            ...
                            <th style="background-color: #2c3e50; color: white; padding: 12px; text-align: center; border: 1px solid #ddd; width: 80px;">Disc %</th>
                    ...
                    <tbody>
                        ${items.map(item => `
                            <tr>
                                ...
                                <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: center; vertical-align: top;">${item.hsn || '-'}</td>
                                <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: center; vertical-align: top;">${item.quantity}</td>
                                <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: right; vertical-align: top;">${appSettings.currencySymbol}${item.price.toFixed(2)}</td>
                                <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: center; vertical-align: top;">${item.discount > 0 ? item.discount + '%' : '-'}</td>
                                <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: right; vertical-align: top;">${appSettings.currencySymbol}${item.total.toFixed(2)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                ...
                <div class="invoice-totals" style="margin-left: auto; width: 300px; border-top: 2px solid #3498db; padding-top: 15px;">
                    <div class="invoice-totals-row" style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        ...
                        <span class="invoice-totals-value" style="font-weight: bold;">${appSettings.currencySymbol}${grandTotal.toFixed(2)}</span>
                    </div>
                    ${amountPaid > 0 ? `
                        <div class="invoice-totals-row" style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                            <span class="invoice-totals-label" style="font-weight: bold;">Amount Paid:</span>
                            <span class="invoice-totals-value" style="font-weight: bold; color: #2e7d32;">${appSettings.currencySymbol}${amountPaid.toFixed(2)}</span>
                        </div>
                        <div class="invoice-totals-row" style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                            <span class="invoice-totals-label" style="font-weight: bold;">Balance Due:</span>
                            <span class="invoice-totals-value" style="font-weight: bold; color: #c62828;">${appSettings.currencySymbol}${balanceDue.toFixed(2)}</span>
                        </div>
                    ` : ''}
                    <div class="invoice-totals-row invoice-grand-total" style="display: flex; justify-content: space-between; margin-top: 12px; padding-top: 8px; border-top: 1px solid #ddd;">
                        ...
                        <span class="invoice-totals-value" style="font-size: 16px; font-weight: bold; color: #3498db;">${appSettings.currencySymbol}${grandTotal.toFixed(2)}</span>
                    </div>
                </div>
        ...
        ${items.length > 0 ? `
            <table class="items-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 10%;" class="text-center">${hsnLabel}</th>
                        <th style="width: 15%;" class="text-center">Quantity</th>
                        <th style="width: 20%;" class="text-right">Unit Price (${appSettings.currencySymbol})</th>
                        <th style="width: 10%;" class="text-center">Disc %</th>
                        <th style="width: 15%;" class="text-right">Amount (${appSettings.currencySymbol})</th>
                    </tr>
                </thead>
                <tbody>
                    ${items.map(item => `
            ...
            <div class="totals-section">
                <div class="total-row">
                    ...
                    <span><strong>${appSettings.currencySymbol}${grandTotal.toFixed(2)}</strong></span>
                </div>
                ${amountPaid > 0 ? `
                    <div class="total-row">
                        <span>Amount Paid:</span>
                        <span class="paid-amount"><strong>${appSettings.currencySymbol}${amountPaid.toFixed(2)}</strong></span>
                    </div>
                    <div class="total-row">
                        <span>Balance Due:</span>
                        <span class="balance-due"><strong>${appSettings.currencySymbol}${balanceDue.toFixed(2)}</strong></span>
                    </div>
                ` : ''}
                <div class="total-row grand-total">
                    <span>${amountPaid > 0 ? 'Total Amount' : 'Amount Due'}:</span>
                    <span class="amount">${appSettings.currencySymbol}${grandTotal.toFixed(2)}</span>
                </div>
            </div>
        ` : `
            <div class="no-items">
        ...
        ${items.length > 0 ? `
            <table class="invoice-table">
                <thead>
                    <tr>
                        <th>Description</th>
                        <th class="text-center">${hsnLabel}</th>
                ...
                <tbody>
                    ${items.map(item => `
                        <tr>
                            ...
                            <td class="text-center">${item.hsn || '-'}</td>
                            <td class="text-center">${item.quantity}</td>
                            <td class="text-right">${appSettings.currencySymbol}${item.price.toFixed(2)}</td>
                            <td class="text-center">${item.discount > 0 ? item.discount + '%' : '-'}</td>
                            <td class="text-right">${appSettings.currencySymbol}${item.total.toFixed(2)}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
            ...
            <div class="invoice-totals">
                ...
                ${amountPaid > 0 ? `
                    <div class="totals-row">
                        <span>Amount Paid:</span>
                        <span class="paid-amount">${appSettings.currencySymbol}${amountPaid.toFixed(2)}</span>
                    </div>
                    <div class="totals-row">
                        <span>Balance Due:</span>
                        <span class="balance-due">${appSettings.currencySymbol}${balanceDue.toFixed(2)}</span>
                    </div>
                ` : ''}
                <div class="totals-row grand-total">
                    <span>${amountPaid > 0 ? 'Total Amount' : 'Amount Due'}:</span>
                    <span class="total-amount">${appSettings.currencySymbol}${grandTotal.toFixed(2)}</span>
                </div>
            </div>
...
async function processInvoicePayment(invoiceId) {
    try {
        ...
        if (typeof loadInvoicesForTable === 'function') loadInvoicesForTable();
        if (typeof loadUnpaidInvoicesForBulk === 'function') loadUnpaidInvoicesForBulk();
...
async function handleStockUpdates(newInvoice, oldInvoiceId) {
    const batch = db.batch();
    let hasUpdates = false;

    // Helper to check if product exists in our local cache
    const isTrackedProduct = (name) => {
        return savedProducts.some(p => p.name === name);
    };

    // If editing, revert old stock first (add back quantity)
    if (oldInvoiceId) {
        try {
            const oldDoc = await db.collection('invoices').doc(oldInvoiceId).get();
            if (oldDoc.exists) {
                const oldInvoice = oldDoc.data();
                if (oldInvoice.items) {
                    for (const item of oldInvoice.items) {
                        if (item.type === 'product' && isTrackedProduct(item.description)) {
                            const productId = item.description.replace(/\//g, '_');
            ...
            console.warn("Error fetching old invoice for stock revert:", e);
        }
    }

    // Deduct new stock (subtract quantity)
    if (newInvoice.items) {
    ...
    if (hasUpdates) {
        ...
        if (typeof loadSavedProducts === 'function') loadSavedProducts();
